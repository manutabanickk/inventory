      ...
      if(inventariable == 0){
          ...
          if(perecedero == 0){
                ...
                '<span class="text-muted">'+especificacion+'</span></td>';
                 ...
                 if($("#chkEditable").is(':checked')){
                 tr_add += '<td align="center">';
                 tr_add += '<div style="display:inline-block;"><select class="form-control select-precio-venta" style="width:80px;display:none">';
                 tr_add += '<option value="'+precios[0]+'">'+precios[0]+'</option>';
                 tr_add += '<option value="'+precios[1]+'">'+precios[1]+'</option>';
                 tr_add += '<option value="'+precios[2]+'">'+precios[2]+'</option></select></div>';
                 tr_add += '<div style="display:inline-block;"><input type="text"  id="pEditable" name="pEditable" class="alonso form-control" style="width:80px;"/></div></td>';
                                     }else{
                 tr_add += '<td align="center">';
                 tr_add += '<div style="display:inline-block;"><select class="form-control select-precio-venta" style="width:80px;">';
                 tr_add += '<option value="'+precios[0]+'">'+precios[0]+'</option>';
                 tr_add += '<option value="'+precios[1]+'">'+precios[1]+'</option>';
                 tr_add += '<option value="'+precios[2]+'">'+precios[2]+'</option></select></div>';
                 tr_add += '<div style="display:inline-block;"><input type="text"  id="pEditable" name="pEditable" class="alonso form-control" style="display:none; width:80px;"/></div></td>';
                                     }
                 ...
                 if(existe==false){
                   ...
                   $('.select-precio-venta:last').change(function(){
                     totales();
                   });
                   
                   
                   $("#chkEditable").on("click", function(){
                   if($("#chkEditable").is(':checked')){
                    swal({
                              title: "Ingresar contraseña Jesus",
                              type: "input",
                              ...
                              if (inputValue != "JGC3004" && inputValue != "jgc3004" ) {
                                swal.showInputError("Contraseña incorrecta!");
                                $( "#chkEditable" ).prop( "checked", false );
                                return false
                              }else{
                                  totales();
                                  swal("Correcto!", "Precios editables habilitados", "success");
                                  $('.select-precio-venta').css("display", "none");
                                  $('.alonso').css("display", "");
                                      
                              }
                            });
                      }else{
                                //    $('.select-precio-venta').css("display", "");
...
$(document).on("focusout","#tblcant",function(){
 ...
 $(document).on("focusout","#tbldesc",function(){
 totales();
})
 $(document).on("focusout","#pEditable",function(){
 totales();
})
...
var total_sumas=0; total_exentos=0; total_iva = 0; subtotal=0; sumas = 0; iva = 0;exentos = 0; total=0;descuentos = 0;
         ...
         $.each(data,function(key,val){
           ...
           iva_div = iva + 1;
            ...
            $("#tbldetalle tbody tr").each(function (index)
                {
                    var campo1=0, campo2=0, campo3=0, campo4=0, campo5=0, campo6=0, campo7=0, campo8=0, campo9=0, campo10=0, campo11=0;
                    $(this).children("td").each(function (index2)
                    {
                        switch (index2)
                        {
                            ...
                            case 3:  if($("#chkEditable").is(':checked')){
                                     campo3 = $(this).find("#pEditable").val();
                                     }else{
                                     campo3 = $(this).find('.select-precio-venta').val();
                                     }
...
z
                            ...
                            case 5:  campo5 = $(this).find("#tbldesc").val();
                                     ...
                                     if(descuentos > calculo_precio)
                                     {
                                      ...
                                      $("#tbldesc").trigger("touchspin.updatesettings", {max: 100000});
                                     } else {
                                      descuentos = parseFloat(campo5);
                                      $("#tbldesc").trigger("touchspin.updatesettings", {max: 100000});
                                     }
...
function enviar_data(){
  ...
  var fecha_vence = "";
    ...
    $("#tbldetalle tbody tr").each(function (index)
        {
            var campo1=0, campo2=0, campo3=0, campo4=0, campo5=0, campo6=0, campo7=0;
            $(this).children("td").each(function (index2)
            {
                switch (index2)
                {
        ...
        if(total > 0.00){
           ...
           swal("Imposible","No se puede registrar una venta","warning");
