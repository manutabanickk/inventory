...
class PDF extends FPDF
{
    ...
    function Header()
    {
        if ($this->page == 1) {
            ...
            $this->Cell(103);
            $this->Cell(105, 10, 'ENTRADAS DE PRODUCTOS DEL MES DE ' . strtoupper($meses[intval(substr($mes, 0, 2)) - 1] . ' del ' . $ano), 0, 0, 'C');
            $this->Ln(20);
        }
    }
...
try {
    ...
    if (is_array($listado) || is_object($listado)) {
        foreach ($listado as $row => $column) {
            ...
            $fecha_movimiento = isset($column["fecha_entrada"]) ? $column["fecha_entrada"] : null;
            ...
            $descripcion_entrada = isset($column["descripcion_entrada"]) ? $column["descripcion_entrada"] : '';
            $cantidad_entrada = isset($column["cantidad_entrada"]) ? $column["cantidad_entrada"] : 0;
            $costo_total_entrada = isset($column["costo_total_entrada"]) ? $column["costo_total_entrada"] : 0;
            ...
            $pdf->Cell(80, 9, $descripcion_entrada, 1, 0, 'L', 1);
            $pdf->Cell(25, 9, $cantidad_entrada, 1, 0, 'C', 1);
            $pdf->Cell(25, 9, $costo_total_entrada, 1, 0, 'C', 1);

            $total_entrada += $cantidad_entrada;
            $total_final += $costo_total_entrada;
