<?php
    ...
    foreach ($datos as $row => $column) {
		...
		$pdf = new TICKET('P','mm',array(76,158));
		$pdf->AddPage();
			...
			if($tipo_comprobante == '1')
			{
				...
				$pdf->Text(22, $get_YH  + 5, 'TICKET ELECTRONICO');
				$pdf->Text(31, $get_YH  + 10, $numero_venta);
				...
				$pdf->Cell(33,4,'Descripcion',0,0,'L',1);
				$pdf->Cell(14,4,'Precio',0,0,'L',1);
				...
				while($row = $detalle->fetch(PDO::FETCH_ASSOC)) {
				 $item = $item + 1;
					...
					$pdf->SetFont('Arial','',6);
					$pdf->Cell(38,4,$row['codigo_barra'].' '.$row['descripcion'],0,0,'L',1);
					$pdf->SetFont('Arial','',8.5);
					$pdf->Cell(12,4,$row['precio_unitario'],0,0,'L',1);
