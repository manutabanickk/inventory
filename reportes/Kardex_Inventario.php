...
try {
    ...
    if (is_array($listado) || is_object($listado)) {
        foreach ($listado as $row => $column) {
            ...
            $total_inicial += floatval($column["saldo_inicial"]);
            $total_entrada += floatval($column["entradas"]);
            $total_salidas += floatval($column["salidas"]);
            $total_final += floatval($column["saldo_final"]);
