...
spl_autoload_register(function ($className) {
    ...
    $datos = $objCredito->Listar_Creditos_Espc_Proveedor($idcredito);

    $list_abonos = $objCredito->Listar_Abonos_Credito_Proveedor($idcredito);

    if (is_array($datos) || is_object($datos))
    {
        foreach ($datos as $row => $column) {
          $codigo_credito = $column["codigo_credito"];
          $idcompra = $column["idcompra"];
          $numero_compra = $column["numero_compra"];
          ...
          $codigo_cliente = $column["codigo_proveedor"];
    ...
    $listado = $objCompra->Listar_Detalle($idcompra);
...
try {
    ...
    $pdf->Text(42, 37, $codigo_cliente.' - '.$proveedor);
    ...
    $pdf->Cell(143,5,'Producto',1,0,'L',1);
    ...
    if (is_array($listado) || is_object($listado))
    {
        ...
        $pdf->Cell(143,7,$column["nombre_producto"].' - '.$column["nombre_marca"],1,0,'L',1);
